# Начало Работы

## Подготовка Проекта

Первым делом, для внесения каких-либо изменений вам необходимо скачать и установить
проект Open vAIR.

Для этого вам понадобиться «чистая» машина соответсвующая системным требованиям ([Системные Требования](../install/system_requirements.md#system-requirements-label)).
Старайтесь, не устанавливать Open vAIR на вашу «рабочую» станцию, так как
возможен вариант возникновения коллизий и конфликтов зависимостей.

Чтобы установить и запустить свой экземпляр Open vAIR,
просто следуйте инструкции по установке ([Установка](../install/install.md#install-osv-label)).
