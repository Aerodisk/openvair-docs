...
Для удобства запуска и отслеживания результатов тестов, при ручном запуске,
рекомендуется настроить плагины для тестирования pytest в IDE(VSCode, PyCharm).
...
- Перед запуском требуется настроенное окружение: PostgreSQL, RabbitMQ, файловая
  директория (`/dev/vdb`)
- Используется `test_storage` c `localfs`, который создаётся и удаляется в
  каждом запуске
