...
1. Получение IQN инициатора:
   ...
   > ![image](_static/img/block_devices/ISCSI/login/1_get_iqn.png)
...
2. Логин в ISCSI устройство:
   ...
   > ![image](_static/img/block_devices/ISCSI/login/2_login.png)
   ...
   > ![image](_static/img/block_devices/ISCSI/login/3_login.png)

   После чего в таблице отобразится созданная сессия:
   > ![image](_static/img/block_devices/ISCSI/login/4_login.png)

   Подключенное ISCSI устройство можно будет увидеть во вкладке «Физические диски»:
   > ![image](_static/img/block_devices/ISCSI/login/5_phys_disks.png)

   Для наглядности, вот как выглядела таблица физических дисков до того как мы подключили ISCSI устройство:
   > ![image](_static/img/block_devices/ISCSI/login/0_phys_disks_before.png)

   И вот как выглядит эта же таблица после подключения ISCSI устройства:
   > ![image](_static/img/block_devices/ISCSI/login/6_phys_disks_after.png)
...
> > ![image](_static/img/block_devices/ISCSI/logout/1_logout.png)

> Появится модальное окно с просьбой подтвердить удаление выбранной сессии, нажимаем «Разлогинить»

> > ![image](_static/img/block_devices/ISCSI/logout/2_logout.png)

> После чего удаленная сессия пропадет из таблицы сессий:

> > ![image](_static/img/block_devices/ISCSI/logout/3_logout.png)
...
> ![image](_static/img/block_devices/FC/0_scan.png)
...
> ![image](_static/img/block_devices/FC/1_scan_modal.png)

После чего на странице «Физические диски» отобразятся подключенные Fibre Channel устройства:

> ![image](_static/img/block_devices/FC/2_phys_disks.png)
